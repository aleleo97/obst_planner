...
class ConvexOpt():
  ...
  def CVXOPT(self,opt_power = False,opt_velocity = False):
    ...
    obj = cp.sum_squares(np.ones(shape=(1,self.N))*tau +np.ones(shape=(1,self.N))*tau_vel + 10*np.ones(shape=(1,self.N-1))*tau_u  + np.ones(shape=(1,self.N-1))*10**3*hogb+ 10**5*nu)
    obj = cp.Minimize(obj)
